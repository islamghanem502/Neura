...
// ================================
// ORGAN RISK HIGHLIGHT SYSTEM
// ================================
const _organOriginalMaterials = new Map();

window.highlightInjuredOrgans = function (organsList) {
    if (!modelRoot) return;

    // Restore all previously highlighted organs first
    _organOriginalMaterials.forEach((mat, mesh) => {
        mesh.material = mat;
    });
    _organOriginalMaterials.clear();

    if (!organsList || organsList.length === 0) return;

    organsList.forEach(function (organData) {
        const riskLevel = Number(organData.risk_level) || 0;
        if (riskLevel < 0.2) return; // Only highlight meaningfully affected organs

        const organKey = (organData.organ || '').toLowerCase();

        modelRoot.traverse(function (child) {
            if (!child.isMesh) return;
            const meshName = child.name.toLowerCase();

            // Match organ name to mesh name (handle plurals / partials)
            const nameMatch =
                meshName === organKey ||
                meshName.startsWith(organKey) ||
                organKey.startsWith(meshName) ||
                (organKey === 'lungs' && meshName === 'lung') ||
                (organKey === 'lung' && meshName === 'lung') ||
                (organKey === 'kidneys' && meshName === 'kidneys') ||
                (organKey === 'kidney' && meshName === 'kidneys');

            if (!nameMatch) return;
            if (!ORGAN_NAMES.includes(child.name)) return;

            // Save original material
            if (!_organOriginalMaterials.has(child)) {
                _organOriginalMaterials.set(child, child.material);
            }

            // Pick color by risk level
            let emissiveColor, emissiveIntensity;
            if (riskLevel > 0.6) {
                emissiveColor = new THREE.Color(0xff2200); // red
                emissiveIntensity = 0.9;
            } else if (riskLevel > 0.35) {
                emissiveColor = new THREE.Color(0xff8800); // amber
                emissiveIntensity = 0.7;
            } else {
                emissiveColor = new THREE.Color(0xffee00); // yellow-mild
                emissiveIntensity = 0.4;
            }

            // Clone the original material and apply emissive glow
            const origMat = _organOriginalMaterials.get(child);
            let newMat;
            if (origMat && origMat.isMeshStandardMaterial) {
                newMat = origMat.clone();
                newMat.emissive = emissiveColor;
                newMat.emissiveIntensity = emissiveIntensity;
            } else {
                // Fallback: MeshPhongMaterial
    ...
    // Animate pulsing for highlighted meshes
    _startRiskPulse();
};

let _riskPulseAnimation = null;
let _riskPulseActive = false;

function _startRiskPulse() {
    if (_riskPulseActive) return;
    _riskPulseActive = true;
    let t = 0;
    function pulse() {
        if (_organOriginalMaterials.size === 0) {
            _riskPulseActive = false;
            return;
        }
        t += 0.05;
        const pulseFactor = 0.5 + 0.5 * Math.sin(t * 3.0);
        _organOriginalMaterials.forEach((origMat, mesh) => {
            if (mesh.material && mesh.material.emissiveIntensity !== undefined) {
                const base = mesh.material.emissiveIntensity;
                // Only modulate, don't fully reset
                mesh.material.emissiveIntensity = 0.2 + pulseFactor * 0.8;
            }
        });
        _riskPulseAnimation = requestAnimationFrame(pulse);
    }
    pulse();
}
