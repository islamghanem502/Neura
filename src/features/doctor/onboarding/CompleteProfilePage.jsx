...
import {
  ...
  Loader2, Lock, BadgeCheck, Shield, Headphones, AlertCircle, ArrowLeft, CheckCircle2, ChevronRight, ArrowRight, Info
} from 'lucide-react';
import {
  useDoctorData,
...
const SIDEBAR_STEPS = [
  ...
  { id: 5, title: 'Verification', icon: CheckCircle },
];
...
export default function CompleteProfilePage() {
  ...
  const onBasicSubmit = (data) => {
    ...
    basicInfoMutation.mutate(payload, { 
      onSuccess: () => {
        profInfoMutation.mutate(profPayload, { onSuccess: () => setCurrentStep(2) });
      } 
    });
  };
  ...
  const handleCombinedUpload = (e) => {
    ...
    if (!docMeta.medicalLicense.licenseNumber || !docMeta.medicalDegree.university || !docMeta.medicalDegree.degree) {
       toast.error('Please fill the text fields above before uploading certificates');
       e.target.value = null;
       return;
    }

    // Upload as Medical License
    const formDataLicense = new FormData();
    formDataLicense.append('file', file);
    formDataLicense.append('licenseNumber', docMeta.medicalLicense.licenseNumber);
    formDataLicense.append('issueDate', '2020-01-01');
    formDataLicense.append('expiryDate', '2030-01-01');
    uploadDocMutation.mutate({ documentType: 'medical-license', formData: formDataLicense });

    // Also copy it as Medical Degree to satisfy verification checks
    const formDataDegree = new FormData();
    formDataDegree.append('file', file);
    formDataDegree.append('university', docMeta.medicalDegree.university);
    formDataDegree.append('graduationYear', '2020');
    formDataDegree.append('degree', docMeta.medicalDegree.degree);
  ...
  const stepDescriptions = {
    ...
    4: "Define your working hours and scheduling availability.",
    5: "Complete the remaining verification procedures."
  };
    ...
    <div className="flex min-h-screen bg-[#f8fafc] font-sans text-slate-800">
      ...
      <aside className="w-64 md:w-72 bg-white border-r border-slate-200 flex flex-col hidden md:flex shrink-0">
        ...
        <div className="px-8 mb-6">
          ...
          <p className="text-[11px] font-semibold text-slate-400 mt-0.5">Step {currentStep} of 5</p>
          ...
          <div className="flex items-center gap-6 ml-6">
            ...
            <div className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-semibold text-slate-700 flex items-center gap-1">
                 Dr. {doctorData?.lastName || 'User'} <ChevronRight size={14} className="text-slate-400 rotate-90" />
              </span>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8 pt-6">
          <div className="max-w-4xl">
            ...
            <div className="pb-20">
                    ...
                    <form onSubmit={handleSubmitBasic(onBasicSubmit)} className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100/60 flex flex-col relative overflow-hidden">
                       ...
                       <div className="space-y-6 flex-1">
                          <div>
                             ...
                             <div className="grid grid-cols-2 gap-3">
                                <input {...registerBasic('firstName')} className={inputClass} placeholder="First Name" />
                                <input {...registerBasic('lastName')} className={inputClass} placeholder="Last Name" />
                             </div>
                          </div>
                          ...
                          <div>
                             ...
                             <input {...registerBasic('phone')} className={inputClass} placeholder="+1 (555) 000-0000" />
                          </div>
                          ...
                          <div className="pt-6 border-t border-slate-100 mt-6 !mb-2">
                             ...
                             <div className="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                   ...
                                   <select {...registerBasic('gender')} className={inputClass + ' appearance-none'}>
                             ...
                             <div className="grid grid-cols-3 gap-3">
                                <input {...registerBasic('address.governorate')} className={inputClass} placeholder="Gov." />
                                <input {...registerBasic('address.city')} className={inputClass} placeholder="City" />
                                <input {...registerBasic('address.street')} className={inputClass} placeholder="Street" />
                             </div>
                             
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-100">
                                <div>
                                  <label className={labelClass}>Years of Experience</label>
                                  <input type="number" {...registerBasic('yearsOfExperience')} className={inputClass} />
                                </div>
                                <div>
                                  <label className={labelClass}>Primary Specialization</label>
                                  <input {...registerBasic('primarySpecialization')} className={inputClass} placeholder="e.g. Cardiology" />
                                </div>
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="flex items-end justify-between mt-8 pt-4">
                          ...
                          <button 
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-full font-semibold text-[13px] flex items-center gap-2 transition-transform active:scale-95 shadow-md shadow-blue-600/20"
                          >
                            {basicInfoMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <>Next Step <ArrowRight size={16} /></>}
                          </button>
                       </div>
                    </form>
                  </div>
              ...
              {currentStep === 2 && (
                <div className="space-y-4 animate-in fade-in duration-500 max-w-2xl">
                  ...
                  <div className="flex justify-between items-center pt-8 border-t border-slate-100">
                    ...
                    <button type="button" onClick={() => setCurrentStep(3)} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-full font-semibold text-[13px] shadow-md shadow-blue-600/20 flex items-center gap-2 transition-all active:scale-95">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex justify-between items-center pt-4">
                        ...
                        <button 
                          ...
                          className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-full font-semibold text-[14px] shadow-md shadow-blue-600/20 flex items-center gap-2 transition-all active:scale-95"
                        >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
                    ...
                    <div className="space-y-6">
                      <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100/60">
                        <div className="flex justify-between items-start mb-6">
                           <div>
                             <h3 className="font-bold text-slate-800 text-[16px] mb-1">Availability Schedule</h3>
                             <p className="text-[13px] text-slate-500 font-medium">Define your standard working hours for this clinic.</p>
                           </div>
                        </div>

                        <div>
                            <div className="flex flex-col gap-3">
                              {clinicForm.availableHours.map((slot, index) => (
                                <div key={index} className="bg-[#f8fafc] p-3 rounded-2xl flex items-center justify-between text-[13px] font-medium text-slate-600 border border-slate-100">
                                  <select 
                                    className="bg-transparent text-[11px] uppercase font-black text-slate-700 focus:outline-none cursor-pointer"
                                    value={slot.day}
                                    onChange={(e) => {
                                      const newHours = [...clinicForm.availableHours];
                                      newHours[index].day = e.target.value;
                                      setClinicForm({...clinicForm, availableHours: newHours});
                                    }}
                                  >
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="text-slate-300 text-xs text-light mx-1">to</span>
                                    <input 
                                      type="time" 
                                      className="bg-transparent font-bold text-slate-800 focus:outline-none" 
                                      value={slot.endTime} 
                                    ...
                                    <button 
                                      ...
                                      onClick={() => {
                                        const newHours = clinicForm.availableHours.filter((_, i) => i !== index);
                                        setClinicForm({...clinicForm, availableHours: newHours});
                                      }}
                                    >
                                      &times;
                              ...
                              <button 
                                type="button" 
                                className="w-max text-[11px] font-bold text-blue-600 hover:text-blue-700 px-2 mt-1 -ml-1 transition-colors"
                                onClick={() => setClinicForm({
                                  ...clinicForm, 
                                  availableHours: [...clinicForm.availableHours, { day: 'Monday', startTime: '09:00', endTime: '17:00' }]
                                })}
                              >
                                + Add Hours
                              </button>
                      ...
                      <div className="flex justify-between items-center pt-4">
                        <button type="button" onClick={() => setCurrentStep(3)} className="text-blue-600 hover:text-blue-700 font-bold flex items-center gap-2 transition-all text-[14px]">
                          <ArrowLeft size={18} /> Previous Step
                        </button>
                        <button 
                          type="button" 
                          onClick={() => {
                            if(clinicForm.address.city || clinicForm.phone || clinicForm.clinicName) {
                              clinicMutation.mutate({ 
                                clinicName: clinicForm.clinicName || 'Primary Practice',
                          ...
                          className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-full font-semibold text-[14px] shadow-md shadow-blue-600/20 flex items-center gap-2 transition-all active:scale-95"
                        >
                          {clinicMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <>Save and Continue <ArrowRight size={18} /></>}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                       <div className="bg-[#f8fafc] rounded-[2rem] p-6 border border-slate-100 shadow-sm">
                          ...
                          <p className="text-[13px] text-slate-500 font-medium leading-relaxed pr-2">
                             These hours dictate when patients can book appointments with you. Keep them updated to avoid scheduling conflicts.
                          </p>
                       </div>
                    </div>
                  </div>
              ...
              {/* Step 5: Verification Placeholder until implementation */}
              {currentStep === 5 && (
                <div className="bg-white rounded-[2rem] p-12 text-center shadow-sm border border-slate-100/60 animate-in fade-in duration-500 max-w-2xl">
                  ...
                  <div className="flex justify-between items-center pt-8 border-t border-slate-100">
                    <button type="button" onClick={() => setCurrentStep(4)} className="text-blue-600 hover:text-blue-700 font-bold flex items-center gap-2 transition-all text-sm">
                      <ArrowLeft size={16} /> Previous Step
                    </button>
                    <button
